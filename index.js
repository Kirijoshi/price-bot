...
const vehicleData = {
  ...
  javelin: { price: 15.0, available: ":white_check_mark:", emoji: ":rocket:" },
...
// Use an ordered list of vehicles if needed (or use Object.keys(vehicleData))
...
client.on("messageCreate", async (message) => {
  ...
  // Deduplication: If this message was already processed, skip it.
  if (processedMessages.has(message.id)) {
    return;
  }
  // Mark the message as processed
  processedMessages.add(message.id);
  // Optionally, clear the processed message after a timeout to avoid memory buildup.
  setTimeout(() => processedMessages.delete(message.id), 60000); // 60 seconds

  const content = message.content.trim();

  // Command to display prices
  if (content === "!prices") {
  ...
  // Command to edit vehicle prices: !editprices <vehicle name> <price>
  if (content.startsWith("!editprices")) {
    const args = content.split(" ").slice(1);
    if (args.length < 2) return message.reply("Usage: `!editprices <vehicle name> <price>`");
    ...
    return message.reply(`✅ Updated price of **${name}** to **$${newPrice}**`);
  }
});
